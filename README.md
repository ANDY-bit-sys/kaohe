...
- 操作：点击“手动新局”，使用方向键或 WASD 转向；手机支持方向按钮、棋盘滑动。空格或暂停按钮用于暂停与继续。手动默认每秒 4 步，也可选择每秒 6 或 10 步，调整即时生效。
...
- AI 使用规则算法，沿覆盖全部格子的闭合路线前进；不调用在线大模型。AI 新局的蛇身按路线顺序摆放，这个初始条件是安全巡航的前提。
- 选择这一方案的原因：路线上的相邻格都能直接走到，而且不会把蛇带出棋盘；蛇身按同一顺序前进，蛇头不会追上仍被身体占据的位置。食物随机生成在空格，巡航最终会经过它。代价是有时会绕路，不追求最短路径。
- 验证：200 组不同种子的随机食物序列全部达到连续 15 个食物，所需步数为 614 至 1389；另有 5 组运行至填满棋盘，全部吃到 140 个食物并获胜。浏览器还使用正常计时和随机食物完成了自动连续吃到至少 15 个的实测。
- 复核方法：AI 默认每秒 8 步，方便观察，也可切换到每秒 20 或 45 步。点击“AI 自动新局”，保持页面可见且不操作，观察挑战进度变为“15 个挑战已达标”；选择“快速 · 45 步/秒”时，200 组测试对应约 14 至 31 秒，浏览器调度可能影响实际时间。
...
2. **AI 为什么能安全巡航？** 程序先把全部格子连成首尾相接的路线。每一步，蛇头都走到路线上的下一个格子；蛇身也沿着相同顺序跟进。只要从程序安排的 AI 新局开始，蛇头前方仍有空位，直到填满棋盘时结束。它依靠路线保证安全，不是每一步都猜哪条路更好。
