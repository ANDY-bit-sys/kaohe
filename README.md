...
- 视觉：当前版本为“芯片寻迹”，使用 Canvas 中的原生 WebGL 绘制三维芯片地图。12 × 12 棋盘位于芯片表面，周围有四组金属引脚、PCB 走线、焊盘和贴片元件；机械蛇带青色节段和指示方向的蛇头，琥珀色信号节点代表食物。深浅主题均适配，画布按显示尺寸与像素密度调整清晰度。外围元件仅作装饰，不改变碰撞规则；没有外部图片或框架依赖。
