...
- 视觉：当前版本为“芯片寻迹”，使用 Canvas 中的原生 WebGL 绘制三维芯片地图。12 × 12 棋盘位于芯片表面，周围有金属引脚、PCB 走线、焊盘和贴片元件；蛇头是混元 3D 生成的深蓝色芯片信号龙，带金色引脚角、面罩笑眼和电路线。蛇身根据实时位置插值成圆滑、渐细的深蓝色曲管，表面贴着青色发光线、金色电路纹和焊盘；琥珀色信号节点代表食物。深浅主题均适配，画布按显示尺寸与像素密度调整清晰度。外围元件仅作装饰，不改变碰撞规则；模型已内嵌，没有外部图片或框架依赖。
- 验证：自动测试覆盖移动、增长、反向输入、撞墙、自撞、进入刚移走的尾格、暂停和重新开局；浏览器检查键盘、按钮、滑动与手机布局。
- 2026-09-21 芯片主题复测：游戏规则与 AI 测试、最近 10 局战绩测试、浏览器实际连续收集 15 个节点均通过；检查深浅主题和 320/390px 手机布局，未出现页面脚本错误。
- 2026-09-25 混元模型接入复测：本地 `file://` 浏览器可直接显示内嵌模型；规则测试 1000 组 AI 连吃 15 个、20 组满棋盘，浏览器实测 15 个食物，手机布局与最近战绩检查通过，未出现页面脚本错误。

#### 三维角色来源

本人希望角色贴合集成电路学院的主题，并提出用混元网页版建模。Codex 先制作[头部参考图](assets/reference/signal-dragon-head.png)，再在本人扫码登录的混元 3D 网页中上传参考图，生成约 5 万三角面的头部模型。[可下载的优化版 GLB](assets/model/signal-dragon-head.glb)保留几何与主要颜色纹理；原始高分辨率 GLB 保存在本地 `output/hunyuan/`。为了让游戏仍能双击单个 HTML 运行，使用[模型优化脚本](scripts/optimize_signal_dragon.py)压缩纹理，再用[内嵌脚本](scripts/embed_signal_dragon.py)将优化版模型放进 `snake.html`。蛇身仍由程序实时绘制，能随移动、增长和转弯变化。
...
以下为代码逻辑说明及本人于 2026-09-08 在问答中给出的理解。

**本人用自己的话解释：**

- 没吃到食物时，添加新蛇头后为什么要删除最后一节？“为了保持长度不变的同时向前移动一格。”
- 找到通往食物的路线后，为什么还要模拟并检查安全？“为了防止头碰到身，并且为了防止吃完食物后被身体围住。”
...
| “可以把风格变成集成电路，比如把地图改为一个芯片” | 让游戏视觉贴合集成电路学院考核 | 2026-09-21 将森林场景替换为三维芯片封装与 PCB 底板，加入四周引脚、走线及贴片元件；蛇改为机械信号蛇，食物改为琥珀色信号节点，页面同步采用青色与深蓝配色。保留原有规则、AI 寻路、手动 2 步/秒和最近 10 局战绩。 |
| “这蛇是什么？怎么一块一块的？” | 保留芯片地图，让蛇更像蛇 | 将方块机械节段替换为连续圆柱与圆润关节，增加大眼睛、分叉蛇信和渐细尾巴，保留青色配色。 |
| “你能不能设计成和我们学院比较有相关的形象” | 让角色有集成电路辨识度 | 设计芯片信号龙：蓝色封装头部、金色引脚角、电路纹路与发光面罩；先生成参考图供本人查看。 |
| “如果你用混元建模，会不会更精致一点”、“那就干”、“用网页版” | 用混元 3D 网页生成更精致的角色 | 本人扫码登录后，上传头部参考图并生成 50k 面数模型；查看预览、下载 GLB，再将优化版头部嵌入单文件游戏。 |
| “能不能用本地脚本控制 Edge” | 改用本地浏览器完成网页操作 | 使用本地 Edge 自动化操作已登录的混元网页，上传参考图和下载模型；未使用账号密码脚本。 |
| “蛇身和画的是不是差得多了一点” | 让游戏蛇身更接近参考图 | 将逐格直管改为沿蛇身路径生成的平滑、渐细曲面，并在曲面上增加连续青色光线、金色走线及焊盘；保留增长和转弯的实时变化。 |
...
文件与技术验证已补齐，两处代码逻辑已由本人通过问答确认，查错过程已由本人确认完成复现；尚需本人复核电路图和计算，并完成群内收集表提交。[讲解练习](docs/self-review.md)可用于准备。
...
这是本次三个电路开发中真实出现的错误。记录由 Codex 复现整理，保留原错误代码和证据；本人于 2026-09-08 确认已复现查错过程。
